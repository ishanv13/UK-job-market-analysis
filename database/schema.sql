-- UK Job Market Intelligence Platform - Database Schema
-- PostgreSQL Schema for storing job market data from Adzuna, Reed, and ONS APIs

-- Drop tables if they exist (for clean re-creation)
DROP TABLE IF EXISTS skills_extracted CASCADE;
DROP TABLE IF EXISTS job_postings CASCADE;
DROP TABLE IF EXISTS ons_vacancies CASCADE;
DROP TABLE IF EXISTS ons_salaries CASCADE;
...
CREATE TABLE job_postings (
    ...
    CONSTRAINT chk_salary_max CHECK (salary_max >= 0),
    CONSTRAINT chk_salary_avg CHECK (salary_avg >= 0),
    CONSTRAINT chk_salary_range CHECK (salary_max >= salary_min OR salary_max IS NULL OR salary_min IS NULL)
);
...
CREATE TABLE ons_vacancies (
    ...
    CONSTRAINT unique_ons_vacancy UNIQUE (period, region, industry)
);
...
CREATE TABLE ons_salaries (
    ...
    CONSTRAINT unique_ons_salary UNIQUE (period, region, occupation)
);
...
COMMENT ON TABLE job_postings IS 'Job postings collected from Adzuna and Reed APIs';
COMMENT ON TABLE skills_extracted IS 'Skills extracted from job descriptions using NLP/regex';
COMMENT ON TABLE ons_vacancies IS 'ONS vacancy statistics by industry and region';
COMMENT ON TABLE ons_salaries IS 'ONS salary statistics by occupation and region';

COMMENT ON COLUMN job_postings.salary_avg IS 'Calculated as (salary_min + salary_max) / 2';
COMMENT ON COLUMN job_postings.role_category IS 'Classified using keyword matching on job title';
COMMENT ON COLUMN skills_extracted.skill_category IS 'Categories: technical, soft, tool';
